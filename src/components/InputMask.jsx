...
export default function InputMask({ data, onChange, sections }) {
  ...
  // Definition aller möglichen Gruppen mit ihren Items
  const allFields = [
    {
      section: 'Basisdaten',
      items: [
    ...
    {
      section: 'Lizenz 1 / Städteserie (C-Hub)',
      items: [
    ...
    {
      section: 'Lizenz 2 / Website & Shop (C-Hub)',
      items: [
  ...
  // Filtere nur die Gruppen, die in `sections` enthalten sind (oder alle, wenn undefined)
  const fields = sections
    ? allFields.filter(group => sections.includes(group.section))
    : allFields;
    ...
    <div className="space-y-6">
      {fields.map((group, gi) => (
        <fieldset key={gi} className="p-4 border rounded-2xl bg-gray-50">
          <legend className="text-lg font-medium mb-2">{group.section}</legend>
              ...
              <div key={item.name} className="flex items-center space-x-4">
                ...
                {item.type === 'month' ? (
                  ...
                  <input
                    ...
                    readOnly
                    value={data[item.name]}
                    className="flex-1 p-1 bg-gray-100"
                  />
                ) : (
